...
Каждый файл поддерживает массив `urls`. Первый `url` остаётся основным, дополнительные адреса используются как резервные.

При публикации можно сразу передать зеркала:
...
Патчер автоматически повторяет загрузку и переключается на следующий адрес при ошибке.
